...
## Run evaluation code
Besides the demo code, we also provide evaluation codes for reconstruciton and keypoint detection. `tools/eval_sv.py` will run complete single view reconstruciton on all instances in `instance_test.json` and evalutate IOU and PCK. `eval_cv.py` run cross view evaluation on the quality of single view reconstruction, using instances that have multiview access `multiview_instance.pth`. For each instance, from each view, it will run reconstruction, perform global rigid alignment, and evaluate on non-source view. 
